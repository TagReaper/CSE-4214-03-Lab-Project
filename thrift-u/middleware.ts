      ...
      switch (decodedToken.role) {
        case "Admin":
          if (
            ...
            !adminPaths.some((item) =>
              request.nextUrl.pathname.includes(item)
            ) &&
            !publicPaths.some((item) => request.nextUrl.pathname.includes(item))
          ) {
            return NextResponse.redirect(
              new URL("/invalidAccess", request.url)
            );
          }
          break;
        case "Buyer":
        ...
        case "Seller":
          if (
            ...
            (decodedToken.status != "approved" ||
              (!sellerPaths.some((item) =>
                request.nextUrl.pathname.includes(item)
              ) &&
                !publicPaths.some((item) =>
                  request.nextUrl.pathname.includes(item)
                )))
          ) {
            return NextResponse.redirect(
