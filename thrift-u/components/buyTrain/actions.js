...
export async function checkout(Address, Payment) {
  if (verifyRole("Buyer")) {
    ...
    try {
      ...
      const Inventory = InventoryRef.docs.map((doc) => ({
        ...doc.data(),
        id: doc.id,
      }));
      ...
      for (let index = 0; index < Items.length; index++) {
        const sellerRef = await getDoc(
          doc(FireData.db, "Seller", Items[index].sellerId)
        );
        const seller = sellerRef.data();
        pendingOrders = seller.pendingOrders;
        const OrderItemRef = await addDoc(
          collection(FireData.db, "OrderItems"),
          {
            orderId: orderRef.id,
...
export async function acceptOrder(
  ...
  if (verifyRole("Seller")) {
    try {
      ...
      const orderItemRef = await getDoc(
        doc(FireData.db, "OrderItems", orderItemId)
      );
      if (orderItemRef.data().status != "pending") {
...
export async function denyOrder(orderItemId, pendingOrders) {
  if (verifyRole("Seller")) {
    try {
      ...
      const itemRef = await getDoc(
        doc(FireData.db, "Inventory", orderItemRef.data().itemId)
      );
      if (orderItemRef.data().status != "pending") {
        return "Accept refuse failed: Something went wrong.";
      }
      await updateDoc(doc(FireData.db, "OrderItems", orderItemId), {
        status: "canceled",
      ...
      await notifService.sendNotification(
        ...
        NotificationType.SYSTEM_WARNING,
        {
          ...
          reason:
            "The seller has canceled your order. You will receive a full refund.",
        }
      );
...
export async function refund(orderItemId, sellerId) {
  if (verifyRole("Buyer")) {
    try {
      ...
      if (orderItemRef.data().status != "pending") {
        return "Order Cancelation failed: Something went wrong.";
      }
      ...
      await notifService.sendNotification(
        ...
        {
          orderId: orderItemId,
          message: "The buyer has requested a refund.",
        }
      );
...
export async function review(orderItemId, sellerId, rating) {
  if (verifyRole("Buyer")) {
    try {
      ...
      if (orderItemRef.data().status == "pending") {
        return "Rate Seller failed: Item is still pending, cancel order or wait until it is shipped to rate this seller.";
      }
      await updateDoc(doc(FireData.db, "Seller", sellerId), {
        reviews: reviews,
      });
